...
const Calender = props => {
  ...
  const Legend = () => (
    <div className="legend">
      <div className="legend-item">
        <div className={`legend-circle ${getStatusColor("present")}`}></div>
        <span>Present</span>
      </div>
      <div className="legend-item">
        <div className={`legend-circle ${getStatusColor("absent")}`}></div>
        <span>Absent</span>
      </div>
      <div className="legend-item">
        <div className={`legend-circle ${getStatusColor("gazetted")}`}></div>
        <span>Gazetted</span>
      </div>
      <div className="legend-item">
        <div className={`legend-circle ${getStatusColor("restricted")}`}></div>
        <span>Restricted Leave</span>
      </div>
    </div>
  )
  const dateColors = [
    { date: new Date(2023, 11, 1), status: "present" },
    { date: new Date(2023, 11, 15), status: "absent" },
    { date: new Date(2023, 11, 18), status: "restricted" },
  ]
  const getDateColor = date => {
    const matchingColor = dateColors.find(
      dc => dc.date.getTime() === date.getTime()
    )
  ...
  const getInitialView = () => {
    if (window.innerWidth >= 368 && window.innerWidth < 1200) {
  ...
  return (
    <React.Fragment>
      ...
      <MetaTags>
        <title>My Calendar | MyCareer App</title>
      </MetaTags>
        ...
        <Col xl={3}>
          <Card>
            {/* <CardBody className="d-grid">
              ...
              <div className="position-relative" style={{ marginLeft: "2%" }}>
                <Select
                  options={dataYears.map(option => ({
                    value: option.years,
                    label: option.years,
                  }))}
                  value={{ value: years, label: years }}
          ...
          <div className="card mt-4 mt-xl-0 mb-0">
            <div className="card-body">
              ...
              <FullCalendar
                ...
                drop={arg => console.log(arg.date)}
                eventContent={eventInfo => (
                  <>
                    <div
                      className={`status-circle ${getStatusColor(
                        eventInfo.event.status
                      )}`}
                    ></div>
                    <div>{eventInfo.timeText}</div>
                    <div className={getDateColor(eventInfo.date)}>
